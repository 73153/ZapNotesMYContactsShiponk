...
- (void)baseInit {
    _notSelectedStar = [UIImage imageNamed:@"off_focus_star"];
    _selectedStar = [UIImage imageNamed:@"on_focus_star"];
    _halfSelectedStar = [UIImage imageNamed:@"half_star"];
...
- (void)handleTouchAtLocation:(CGPoint)touchLocation {
    ...
    _rating = 0;
    for(int i = _starViews.count - 1.0; i >= 0; i--) {
        UIImageView *imageView = [_starViews objectAtIndex:i];        
        if (touchLocation.x > imageView.frame.origin.x) {
            ...
            objFeedback = [[Feedback_And_Review_ViewController alloc]init];

            objappsharemanager.ratingStr = [NSString stringWithFormat:@"%f",_rating];
            NSLog(@"%@",objappsharemanager.ratingStr);
            
            [[NSNotificationCenter defaultCenter] postNotificationName:@"SetStare" object:nil];
